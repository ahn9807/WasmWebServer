...
__attribute__((used)) void start_main()
{
...
__attribute__((used)) void *server_main(void *args_in)
{
...
__attribute__((used)) void *server_worker(void *message_fd_in)
{
	...
	while (1)
	{
		...
		if(fd < 0) {
			sys_server_error("open failed");
			sys_exit(-1);
		}
		p = sys_mmap(0, filesize, PROT_READ, MAP_PRIVATE, fd, 0);
		if (p < 0) {
			sys_server_error("mmap failed");
			sys_exit(-1);
		}
		sys_write(childfd, "HTTP/1.1 200 OK\r\n\r\n", sizeof("HTTP/1.1 200 OK\r\n\r\n") - 1);
		sys_write_direct(childfd, p, filesize);
